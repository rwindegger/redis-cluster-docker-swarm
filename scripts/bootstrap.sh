...
docker service create --network redis --name redis-zero redis:alpine

echo "Starting services"
docker stack deploy -c scripts/docker-compose.yml cache

until [ "$(docker run --rm --network redis rwindegger/redis:utils  \
...
old_master=$(docker run --rm --network redis rwindegger/redis:utils \
...
docker run --rm --network redis rwindegger/redis:utils \
